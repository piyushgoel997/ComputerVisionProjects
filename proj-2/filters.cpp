...
int sobolX3x3(const cv::Mat& src, cv::Mat& dst) {
...
int sobolY3x3(const cv::Mat& src, cv::Mat& dst) {
...
int sobolOrientation(cv::Mat& sx, cv::Mat& sy, cv::Mat& dst) {
	sx.convertTo(sx, CV_64FC3);
	sy.convertTo(sy, CV_64FC3);
	auto PI = atan(1) * 4;
	for (int i = 0; i < sx.rows; ++i) {
		for (int j = 0; j < sx.cols; ++j) {
			for (int k = 0; k < 3; ++k) {
				dst.at<cv::Vec3d>(i, j)[k] = (atan2(sy.at<cv::Vec3d>(i, j)[k], sx.at<cv::Vec3d>(i, j)[k]) + PI)/(2*PI);
			}
		}
	}
	return 0;
